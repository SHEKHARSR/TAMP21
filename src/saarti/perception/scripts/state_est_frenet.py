...
# publishes: 
# state (topic /state) - full state containing both frenet and cartesian coordinates (dir in callback to minimize delay)
...
class StateEstFrenet:
    ...
    def __init__(self):
        ...
        # wait for messages before activating
    ...
    def update_frenet_state(self):
        ...
        self.state_out.rhof = self.state_cart.rhof
        self.state_out.rhor = self.state_cart.rhor        
    ...
    def state_cart_callback(self, msg):
        ...
        if(self.received_pathglobal == True):
            # timing
            start = time.time()
